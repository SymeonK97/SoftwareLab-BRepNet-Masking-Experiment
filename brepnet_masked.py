...
class BRepNet(LightningModule):
    ...
    def add_model_specific_args(parser):
        ...
        parser.add_argument("--dropout", default=0.3, type=float, help="If using dropout then this is the dropout probability") #default=0.3
        ...
        parser.add_argument("--use_face_grids", type=int, default=1, help="Use UV-Net style face grids") #default=1
        ...
        parser.add_argument("--use_coedge_grids", type=int, default=1, help="Use UV-Net style edge grids for both coedges, aligned with the coedge direction.") #default=1
        ...
        parser.add_argument('--batch_size', type=int, default=200, help="Number of breps in one batch")  #default=200
    ...
    def forward_embedding(self, Xf, Xe, Xc, Kf, Ke, Kc, Ce, Cf, Csf):
        """
        Forward pass through the network layers and output layer to generate face embeddings.
        This method helps to create embeddings with different versions of Xc.
        """
    ...
    def create_face_embeddings(self, Xf, Gf, Xe, Ge, Xc, Gc, Kf, Ke, Kc, Ce, Cf, Csf):
        ...
        # !Change! Mask a percentage of coedges to create self-supervision task 
        Xf_masked = Xf.clone()
        masking_ratio_f = 0.1   # Set masking ratio for SSL task (15% by default)
        mask_indices_f = None
        if self.training and masking_ratio_f > 0:
            num_fc = Xf.size(0)
            mask_indices_f = torch.randperm(num_fc)[:int(masking_ratio_f * num_fc)]
            mask_indices_f = mask_indices_f[mask_indices_f < num_fc]

            #Noise masking
            #noise_f = torch.randn_like(Xf[mask_indices_f]) * 0.2
            #Xf_masked[mask_indices_f] = Xf[mask_indices_f] + noise_f 

            #block_size_f = int(masking_ratio_f * num_fc)
            #start_index_f = torch.randint(0, num_fc - block_size_f + 1, (1,)).item()
            #Xf_masked[start_index_f:start_index_f + block_size_f] = 0            

            Xf_masked[mask_indices_f] = 0
        #!end!
        ...
        #final_mask_ratio = 0.4
        #current_ratio = initial_mask_ratio + (final_mask_ratio - initial_mask_ratio) * (epoch / max_epochs)
        masking_ratio_e = 0.1  # Set masking ratio for SSL task (15% by default)
        mask_indices_e = None
        if self.training and masking_ratio_e > 0:
            num_edges = Xe.size(0)
            mask_indices_e = torch.randperm(num_edges)[:int(masking_ratio_e * num_edges)]
            mask_indices_e = mask_indices_e[mask_indices_e < num_edges]

            #noise_e = torch.randn_like(Xe[mask_indices_e]) * 0.1
            #Xe_masked[mask_indices_e] = Xe[mask_indices_e] + noise_e

            #Block masking
            block_size_e = int(masking_ratio_e * num_edges)
            start_index_e = torch.randint(0, num_edges - block_size_e + 1, (1,)).item()
            Xe_masked[start_index_e:start_index_e + block_size_e] = 0

            #Xe_masked[mask_indices_e] = 0
        #!end!
        ...
        # !Change! Mask a percentage of coedges to create self-supervision task 
        masking_ratio = 0.2 # Set masking ratio for SSL task (15% by default)
        mask_indices = None
        if self.training and masking_ratio > 0:
            ...
            #Noise masking
            #noise_c = torch.randn_like(Xc[mask_indices]) * 0.1
            #Xc_masked[mask_indices] = Xc[mask_indices] + noise_c  # Zero-out masked coedges
        ...
        #Zero out mask
        #Xc_masked[mask_indices] = 0
        #!end!
        #print(f"Batch Xe: {Xe}")
        #print(f"Batch Xf: {Kf}")
        #print(f"Batch Xf_masked: {Xf_masked}")
        face_embeddings = self.forward_embedding(Xf, Xe, Xc, Kf, Ke, Kc, Ce, Cf, Csf)
        face_embeddings_masked = self.forward_embedding(Xf_masked, Xe_masked, Xc_masked, Kf, Ke, Kc, Ce, Cf, Csf)
        return face_embeddings, face_embeddings_masked, mask_indices
    ...
    def brepnet_step(self, batch, batch_idx, save_segmentation_output):
        ...
        face_embeddings, face_embeddings_masked, mask_indices = self.create_face_embeddings(Xf, Gf, Xe, Ge, Xc, Gc, Kf, Ke, Kc, Ce, Cf, Csf)
        ...
        #segmentation_scores_masked = self.classification_layer(face_embeddings_masked)

        #embedding_difference = face_embeddings - face_embeddings_masked
        # Print the difference
        #print(f"Embedding Difference (Batch {batch_idx}):", embedding_difference)
        ...
        # Now find the loss
        labels = batch["labels"]
        segmentation_loss  = self.find_loss(segmentation_scores, labels)

        #segmentation_loss_masked  = self.find_loss(segmentation_scores_masked, labels)
        # Find the network predictions
        predicted_classes = self.find_predicted_classes(segmentation_scores)
        #predicted_classes = self.find_predicted_classes(segmentation_scores_masked)

        #!Change!
        # SSL Loss calculation
        ssl_weight = 0.2
        #mask_loss = 1 - similarity.mean()
        #mask_loss = F.smooth_l1_loss(face_embeddings, face_embeddings_masked, reduction='mean')
        mask_loss = F.mse_loss(face_embeddings, face_embeddings_masked, reduction='mean')
        #threshold = 0.03
        #if mask_loss.item() < threshold:
        #    adjusted_mask_loss = mask_loss * (mask_loss / threshold)  # Scale down proportionally
        #else:
        #    adjusted_mask_loss = mask_loss

        #print(f"mask_loss (Batch {batch_idx}):", mask_loss.item())  # Print batch-level mask loss
        #print(f"segmentation_loss (Batch {batch_idx}):", segmentation_loss.item())  # Print batch-level mask loss

        total_loss = segmentation_loss  + ssl_weight * mask_loss
        #total_loss =  segmentation_loss_masked
        #Only masking
        #total_loss = ssl_weight * mask_loss
        #print(f"mask_loss (Batch {batch_idx}):", mask_loss.item())  # Print batch-level mask loss
        #print(f"total_loss (Batch {batch_idx}):", total_loss.item())  # Print batch-level mask loss
        #!End!
        ...
        return {
            #!Change!
            "total_loss": total_loss,  # This will be used for backpropagation
            #!End!
            "segmentation_loss": segmentation_loss,
            #"segmentation_loss_masked": segmentation_loss_masked,
            "mask_loss": mask_loss,
            "accuracy": accuracy,
            "iou_data": iou_data
        }
